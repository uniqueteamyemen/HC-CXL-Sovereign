...
HC-CXL v2.1R — Enhanced triple-layer separation pipeline
...
function Invoke-Layer1_RawBuild {
    ...
    try {
        ...
        $workFiles = Get-ChildItem -Path $WORK -File -ErrorAction Stop
        ...
        if ((Get-ChildItem -Path $LAYER1_BUILD -File -ErrorAction SilentlyContinue).Count -gt 0) {
            if ($Force) { Remove-Item -Path (Join-Path $LAYER1_BUILD "*") -Recurse -Force }
            else {
        ...
        Audit-SeparatedEvent -Layer "Layer1" -Event "RAW_BUILD_COMPLETE" -Details "Raw build complete" -Outcome "SUCCESS" -Data @{package_hash=$pkgHash; files_count=$copied.Count}
...
function Invoke-Layer2_ScientificVerification {
    ...
    try {
        ...
        Audit-SeparatedEvent -Layer "Layer2" -Event "SCIENTIFIC_VERIFICATION_COMPLETE" -Details "Scientific verification passed" -Outcome "SUCCESS" -Data @{package_hash=$pkgHash; files_count=$result.verification.files_present.Count}
